...
s0 = torch.tensor([0.01, 0.015])
vol0 = torch.tensor([0.01, 0.005])
lower_vol0 = vol0
upper_vol0 = torch.tensor([0.5, 0.495])
...
for i in range(n_der+1):
    ...
    #Valuation
    prices[0, i] = torch.exp(-r * T) * torch.mean(geom_payoff(s[1:, :], K, T))
...
#%%
#Test data
s0_tensor = torch.ones((n_der+1, n_assets)) * s0 * h
vol0_tensor = torch.ones((n_der+1, n_assets)) * s0 * h
s0_tensor[0, :] = s0
vol0_tensor[0, :] = vol0
corr = torch.corrcoef(torch.transpose(s0_tensor, 0, 1))
x = torch.cat((s0_tensor, vol0_tensor), 1).requires_grad_(True)
...
print('--------------------')
print('Monte-Carlo:')
print('\tPrice error mean:\t\t\t' + str(error_mc_price_mean.item()))
print('\tPrice error std:\t\t\t' + str(error_mc_price_std.item()))
if greeks:
    print('\tGreeks error mean:\t\t\t' + str(error_mc_greeks_mean.item()))
    print('\tGreeks error std:\t\t\t' + str(error_mc_greeks_std.item()))
print('-----')
print('Differential Machine Learning:')
print('\tPrice error mean:\t\t\t' + str(error_dml_price_mean.item()))
print('\tPrice error std:\t\t\t' + str(error_dml_price_std.item()))
if greeks:
    print('\tGreeks error mean:\t\t\t' + str(error_dml_greeks_mean.item()))
    print('\tGreeks error std:\t\t\t' + str(error_dml_greeks_std.item()))